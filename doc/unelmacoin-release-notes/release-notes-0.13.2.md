UnelmaCoin Core version 0.13.2 is now available from:

  <https://download.unelmacoin.org/unelmacoin-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a unelmacoin
...
ECDSA signatures inside UnelmaCoin transactions now use validation using
...
Previous versions of UnelmaCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of UnelmaCoin Core was to raise the default minimum
relay fee.

UnelmaCoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
UnelmaCoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running unelmacoind without having to do any manual configuration.
...
UnelmaCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. UnelmaCoin Core will relay transactions with
...
entirely in UnelmaCoin Core 0.13, and it is at this time undecided whether the
...
UnelmaCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
UnelmaCoin Core automatically creates a hidden service to listen on, without
manual configuration. UnelmaCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if UnelmaCoin Core is listening, and
...
UnelmaCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals UnelmaCoin
Core to use floating fees. By default, UnelmaCoin Core will use floating
fees.
...
(default: `0.0002` UNC/kB).

At all times, UnelmaCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) UNC.
Furthermore, UnelmaCoin Core will never create transactions paying less than
...
`unelmacoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /unelmacoinrpc>
        ...
        # optional bypass unelmacoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< unelmacoinrpc:<password>
    </Location>
...
Restarting unelmacoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in unelmacoin.conf
...
unelmacoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/unelmacoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The UnelmaCoin Core code base has
...
- `unelmacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `unelmacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the UnelmaCoin scripting system that in combination with
...
UnelmaCoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of UnelmaCoin Core.
...
The two phases are distinct in the UnelmaCoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming UnelmaCoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for UnelmaCoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each UnelmaCoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of unelmacoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of unelmacoins being spent is exactly
...
- **Increased security for multisig:** UnelmaCoin addresses (both P2PKH addresses
  ...
  believed to be provided by UnelmaCoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of UnelmaCoin's current ledger (called the UTXO set),
  ...
  note that UnelmaCoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  UnelmaCoin users to individually opt-in to almost any change in the UnelmaCoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and UnelmaCoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the UnelmaCoin
client to UnelmaCoin Core 0.13.2 to signal for segwit support. When a
...
Since UnelmaCoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `unelmacoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `unelmacoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
