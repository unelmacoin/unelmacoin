...
def main():
    ...
    tmpdir = "%s/unelmacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and unelmacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "unelmacoind"]) is not None:
            print("%sWARNING!%s There is already a unelmacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "UNELMACOIND" not in os.environ:
        os.environ["UNELMACOIND"] = build_dir + '/src/unelmacoind' + exeext
        os.environ["UNELMACOINCLI"] = build_dir + '/src/unelmacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `unelmacoin-cli help` (`rpc_interface.txt`).
